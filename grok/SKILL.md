...
**Collision-safe write.** The name is derived from the source stem, so it can collide with a file you didn't create. Before writing the final HTML, if the target path already exists, read its first lines: a prior grok artifact carries `<!-- generated by grok … -->` there (older ones show the unmistakable grok skeleton — tab bar, warm-paper `:root` tokens, Roman-numeral openers). A prior grok artifact → overwrite it; re-generating the same source is the intended path and git holds the history. Anything else — a hand-written page, another tool's output, an unrelated file that merely shares the derived name — is not yours to clobber: write to `<stem>-grok.html` (then `-grok-2`, `-grok-3`, … if those exist too) and tell the user the original was kept and where the new report landed. When you can't tell, treat it as foreign and use the suffix.
...
- **Target path already exists** → collision-safe write (see **Collision-safe write** under § Quick start): overwrite a prior grok artifact, suffix `-grok` for any foreign file, never silently clobber. Leave `_drafts/` untouched.
