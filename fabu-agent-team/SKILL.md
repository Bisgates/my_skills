...
**Model choice (fabux default):** Use `fabux exec -m <slug>` for all work by default. The strategy is **`k3` first, `qwen3.8-max` fallback** — both from fabux (company account). k3 is reliable for infra / execution / eval / GPU work; if k3 exits with rc≠0 and no `final.md`, re-dispatch the same prompt on `qwen3.8-max` with "复用已有产物续跑" (qwen ran 11 such tasks with zero crashes and obeys file-scope rules).

**User's codex account (sol/terra/luna) — opt-in only:** Do **not** use `codex exec` by default. The user's personal codex account has access to `sol` / `terra` / `luna` models but not `k3`/`qwen`. These are available **only if the user explicitly requests them** in the campaign design (e.g. "用我的 codex 账号的 sol" or "用 codex 的 terra"). When the user names sol/terra/luna and says to use their codex account, switch that worker to `codex exec -m gpt-5.6-sol` (or terra/luna) instead of fabux. Otherwise, stick to fabux's k3/qwen.
