(function (factory) {
  ...
  // SPELL-OUT PATCH: when an inlined <template id="katex-shadow-css"> is present
  // (single-file build), use its CSS inside each d-math shadow root instead of an
  // external stylesheet link; otherwise fall back to the upstream CDN link.
  const _soShadowCssEl = (typeof document !== 'undefined') && document.getElementById('katex-shadow-css');
  // A <template>'s CSS lives in its .content fragment, so .textContent on the
  // element itself is empty — read the fragment (fall back to innerHTML/textContent).
  const _soShadowCss = _soShadowCssEl &&
    ((_soShadowCssEl.content && _soShadowCssEl.content.textContent) ||
     _soShadowCssEl.innerHTML || _soShadowCssEl.textContent);
  const katexCSSTag = _soShadowCss
    ? ('<style>' + _soShadowCss + '</style>')
...
`);
  ...
  class DMath extends Mutating(T(HTMLElement)) {
    ...
    static addKatex() {
      // SPELL-OUT PATCH: katex is vendored & loaded inline; skip the external fetch.
      // Defer the render to DOMContentLoaded / next tick so EVERY <d-math> has
      // upgraded first (calling katexLoadedCallback synchronously during the
      // first element's upgrade throws "renderContent is not a function").
      if (typeof katex !== 'undefined') {
        DMath.katexAdded = true;
        if (!DMath._soScheduled) {
          DMath._soScheduled = true;
          const cb = function () { DMath.katexLoadedCallback(); };
