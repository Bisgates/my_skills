...
- **All math is KaTeX** — equations, **and** lab readouts, control labels, color legends, section titles, lab headers. **No plain-text math symbols** anywhere the reader sees (no literal `√ᾱ_t`, `ε_θ`). KaTeX CDN pinned (`katex@0.16.11`), auto-render on `DOMContentLoaded` with `ignoredTags:[...,"pre","code"]`, never `onload`. **Never put a bare `<`/`>` inside `$…$` — the HTML parser eats `<s`/`<m` as tags and silently breaks that formula and all after it; write `\lt`/`\gt` (or `1\le s\le m-2`). Grep the page for raw `<`/`>` in math before shipping.** (full rule in `references/page_authoring.md` § KaTeX.)
