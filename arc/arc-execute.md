...
8. After driving the last step:
   ...
   **Fast-done route.** Eligible only when **all** of these hold:
   - `1_objective.md` acceptance (§03 验收 — both the L1 metrics and the L2 evidence list) is unambiguously met by what is already logged in `0_meta.md` (numbers / paths visible, not "probably good").
   ...
   - Tell the user one short line: "Plan complete; acceptance met. OK to mark this arc done? (I'll write `9_summary.md`.)"
   ...
   - On confirmation: dispatch the reporter per `reporter.md` Phase B (background, `general-purpose`, no explicit `model`, `run_in_background: true`). Tell the user one line: "Drafting `9_summary.md` in the background; will mark done when it lands." Then yield. The completion notification handler (`reporter.md` Phase C) flips the status.
...
9. **Reporter completion notification handling.** See `reporter.md` Phase C — single shared handler for both routes. The handler verifies the file, calls `arc status <id> done`, and tells the user one line. Do not duplicate the logic here.
