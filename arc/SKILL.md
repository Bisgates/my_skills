...
- **Reporter agent.** During `/arc-plan` and `/arc-execute`, the main agent maintains short notes in `_tmp/report_notes.md` (bullet jots — strategy, smoke result, key decisions, load-bearing artifacts). After the final `[done] all plan steps complete` log line, the main agent dispatches **one** `general-purpose` sub-agent **in the background (`run_in_background: true`, non-negotiable)** to fill `templates/7_task_report.html` and write it to `<arc>/7_task_report.html`. **Reporter must never block the main flow** — the user's next instruction (typically `/arc-finalize`) is responded to immediately; the HTML auto-opens via `open` when the background completion notification arrives, possibly several turns later. Section spine: §1 结果 / §2 过程 (短) / §3+ 自由发挥 (轻 — pick 0-4 of the candidates the template ships with). The reporter is **not** user-invocable; full protocol in `reporter.md`.
