...
arcs/all/<id>_<slug>/
  ...
  _drafts/report_notes.md   # in-flight reporter staging notes (appended during plan + execute)
...
**No loose `.py / .html / .json / .ply` at the arc root.** All code goes under `utils/` or `scripts/`; all artifacts go under `output/`. **One exemption:** `7_task_report.html` is the reporter's single-file HTML output and lives at the arc root by design.
